...
import { searchAssetBuilder, withExif, withOwner } from 'src/utils/database';
...
export class AssetRepository implements IAssetRepository {
  ...
  async getByChecksum(libraryId: string, checksum: Buffer): Promise<AssetEntity | null> {
    const item = await this.prismaRepository.assets.findFirst({ where: { libraryId, checksum: checksum } });
